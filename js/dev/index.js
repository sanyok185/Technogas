import { d as dataMediaQueries, s as slideToggle, a as slideUp, b as bodyLock, c as bodyUnlock, e as bodyLockStatus, g as getDigFormat, u as uniqArray } from "./app.min.js";
function spollers() {
...
function initSliders() {
  if (document.querySelector(".portfolio__slider")) {
    new Swiper(".portfolio__slider", {
      ...
      // effect: "fade",
      // fadeEffect: {
      // 	crossFade: true, // або false, якщо хочеш, щоб зникало до того, як з'явиться новий слайд
      // },
      // speed: 1800,
      /*
      // Ефекти
...
class Popup {
  ...
  close(selectorValue) {
    ...
    this.previousOpen.element.removeAttribute(
      this.options.classes.popupActive
    );
  ...
  _openToHash() {
    ...
    if (openButton) {
      this.youTubeCode = openButton.getAttribute(
        this.options.youtubeAttribute
      ) ? openButton.getAttribute(this.options.youtubeAttribute) : null;
    }
...
document.addEventListener("click", function(e) {
  const image = e.target.closest('[data-fls-popup-link="popup-image"]');
  if (image && image.tagName === "IMG") {
    const imgSrc = image.getAttribute("src");
    const popupImg = document.getElementById("popup-image-target");
    if (popupImg && imgSrc) {
